...
export { INTERNAL_H, INTERNAL_W } from './config/constants'
...
const spawnZombie = (state: GameState) => {
  ...
  const baseHp = 10 * 1.18 ** (state.wave - 1)
  const baseReward = 2 * 1.12 ** (state.wave - 1)
...
export const calculateClickDamage = (state: GameState, isCrit: boolean): number => {
  ...
  const levelMultiplier = 1.12 ** (state.upgrades.clickLevel - 1)
...
export const calculateSkillDamage = (state: GameState, skillId: string): number => {
  ...
  switch (skillId) {
    case 'sunBurst': {
      let sbDamage = baseDamage * 5 * 1.15 ** (state.abilities.sunBurst.level - 1)
      if (state.abilities.sunBurst.evolutions.includes('double_burst')) sbDamage *= 1.5
      return sbDamage
    }

    case 'poisonCloud':
      return baseDamage * 0.5 * 1.15 ** (state.abilities.poisonCloud.level - 1)

    case 'grass': {
      let grassDamage = 5 * 1.15 ** state.upgrades.grassLevel
...
export const applyDamageToZombie = (
  state: GameState,
  zombie: Zombie,
  amount: number,
...
export const getEvolutionSpeed = (evolutionSpeedLevel: number): number => {
  return 5 * 1.5 ** (evolutionSpeedLevel - 1)
}
...
const runUpdateStep = (state: GameState, dt: number) => {
  ...
  const requiredProgress = 100 * 1.2 ** totalEvolutions
  ...
  if (
    ...
    && state.abilities.sunBurst.cooldown <= 0
    && state.zombies.length > 0
  ) {
    state.abilities.sunBurst.cooldown = getCooldownForLevel(
      state.abilities.sunBurst.maxCooldown,
      state.abilities.sunBurst.level,
    )

    const damage = calculateSkillDamage(state, 'sunBurst')
    let radius = 1.0
    ...
    state.zombies.forEach(z => {
      ...
      for (let k = 0; k < 5; k++) {
        state.particles.push({
          ...
          x: z.x,
          y: z.y,
          vx: (Math.random() - 0.5) * 400 * radius,
          vy: (Math.random() - 0.5) * 400 * radius,
          life: 0,
          maxLife: 0.8,
          color: '#fef08a',
          size: 5,
  ...
  if (
    ...
    && state.abilities.rootEntangle.cooldown <= 0
    && state.zombies.length > 0
  ) {
    state.abilities.rootEntangle.cooldown = getCooldownForLevel(
      state.abilities.rootEntangle.maxCooldown,
      state.abilities.rootEntangle.level,
    )
  ...
  if (
    ...
    && state.abilities.poisonCloud.cooldown <= 0
    && state.zombies.length > 0
  ) {
    state.abilities.poisonCloud.cooldown = getCooldownForLevel(
      state.abilities.poisonCloud.maxCooldown,
      state.abilities.poisonCloud.level,
    )
  ...
  if (state.upgrades.grassLevel > 0) {
    ...
    if (state.timers.lastGrassTick >= tickRate) {
      ...
      const grassDamage = calculateSkillDamage(state, 'grass')
      ...
      for (let i = state.zombies.length - 1; i >= 0; i--) {
        ...
        if (Math.random() > 0.3) {
          state.particles.push({
            ...
            life: 0,
            maxLife: 0.4,
            color: state.upgrades.grassEvolutions.includes('poison_grass') ? '#a855f7' : '#4ade80',
            size: 4,
          })
        }
      }
  ...
  if (state.timers.lastSunSpawn >= state.timers.nextSunSpawnInterval) {
    ...
    let baseReward = 50 * 1.3 ** state.wave
  ...
  if (state.timers.lastShot >= shotInterval) {
    ...
    // Find target
    const target = state.zombies.reduce(
      (closest, z) => {
        if (!closest) return z
  ...
  if (
    state.timers.lastSpawn >= 1 / state.waveState.spawnRate
    && state.waveState.spawned < state.waveState.totalToSpawn
  ) {
...
export const handleCanvasClick = (
  ...
  for (let i = state.zombies.length - 1; i >= 0; i--) {
    const z = state.zombies[i]
    if (
      internalX > z.x - z.size
      && internalX < z.x + z.size
      && internalY > z.y - z.size * 1.5
      && internalY < z.y + z.size * 0.5
    ) {
...
export const getUpgradeCost = (type: string, level: number, _state: GameState) => {
  ...
  switch (type) {
    case 'damage':
      return 10 * 1.15 ** (level - 1)
    case 'speed':
      return 25 * 1.2 ** (level - 1)
    case 'click':
      return 10 * 1.15 ** (level - 1)
    case 'energy':
      return 50 * 1.25 ** (level - 1)
    case 'evolutionSpeed':
      return 100 * 1.3 ** (level - 1)
    case 'grass':
      return 50 * 1.4 ** level
    case 'sunBurst':
    ...
    case 'poisonCloud':
      return 300 * 1.5 ** level
    case 'solGenerator':
      return 1000 * 1.6 ** level
    default:
      return 999999999
  }
}

export const getUpgradeCostTotal = (
  type: string,
  level: number,
  state: GameState,
  amount: number | 'MAX',
...
export const buyUpgrade = (state: GameState, type: string, amount: number | 'MAX' = 1) => {
  ...
  while (amount === 'MAX' || bought < amount) {
    ...
    if (state.energy >= cost) {
          ...
          if (type === 'sunBurst') {
            state.modal.options = [
              ...
              { id: 'double_burst', name: 'Double Burst', description: 'Deals 50% more damage' },
              {
                id: 'lingering_damage',
                name: 'Solar Flare',
                description: 'Leaves a lingering burn effect',
              },
            ]
            ...
            state.modal.options = [
              ...
              { id: 'stronger_slow', name: 'Thick Vines', description: 'Increases slow effect' },
              {
                id: 'damage_over_time',
                name: 'Thorny Roots',
                description: 'Deals damage while slowed',
              },
            ]
            ...
            state.modal.options = [
              { id: 'wider_cloud', name: 'Toxic Smog', description: 'Affects more enemies' },
              {
                id: 'faster_ticks',
                name: 'Corrosive Acid',
                description: 'Damage ticks twice as fast',
              },
            ...
            state.modal.options = [
              ...
              { id: 'faster_fall', name: 'Comet Suns', description: 'Suns fall twice as fast' },
              {
                id: 'auto_collect',
                name: 'Magnetic Field',
                description: 'Suns are collected automatically',
              },
            ]
          }
        }
            ...
            state.modal.options = [
              {
                id: 'poison_grass',
                name: 'Toxic Weeds',
                description: 'Grass deals 50% more damage and turns purple',
              },
              {
                id: 'faster_ticks',
                name: 'Razor Blades',
                description: 'Grass damages twice as fast',
              },
              {
                id: 'slow_thorns',
                name: 'Entangling Thorns',
                description: 'Grass slightly slows enemies',
              },
            ]
          }
        }
      ...
      switch (type) {
        case 'damage':
          state.plant.damageMultiplier = 1.15 ** (state.upgrades.damageLevel - 1)
          break
        case 'speed':
          state.plant.attackSpeedMultiplier = 1.1 ** (state.upgrades.speedLevel - 1)
          break
        case 'energy':
          state.energyMultiplier = 1.12 ** (state.upgrades.energyLevel - 1)
          break
      }

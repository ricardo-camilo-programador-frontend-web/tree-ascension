...
import { type AudioSettings, getAudioSettings, updateAudioSettings } from './audio'
...
export default function App() {
  ...
  const [confirmModal, setConfirmModal] = useState<{
    ...
    destructive?: boolean
    lang: Language
  }>({ open: false, title: '', message: '', onConfirm: () => {}, onCancel: () => {}, lang: 'en' })
  ...
  useEffect(() => {
    ...
    const uiInterval = setInterval(() => {
      ...
      if (gameState.current.waveCompleted) {
        ...
        showToast(
          `${t[currentLang].wave} ${gameState.current.lastCompletedWave} ${t[currentLang].waveComplete || 'Complete!'}`,
          'success',
        )
  ...
  const confirmReset = () => {
    ...
    if (success) {
      ...
      setUiState(mapStateToUI(gameState.current))
      showToast(
        `${t[lang].prestigeActivated} +${formatNumber(gameState.current.energy)} ${t[lang].startingEnergy}`,
        'info',
      )
    }
  }
  ...
  const getSkillName = (id: string | null) => {
    switch (id) {
      case 'sunBurst':
        return 'Sun Burst'
      case 'rootEntangle':
        return 'Root Entangle'
      case 'poisonCloud':
        return 'Poison Cloud'
      case 'solGenerator':
        return 'Sol Generator'
      case 'grass':
        return 'Sharp Grass'
      case 'damage':
        return 'Plant Damage'
  ...
  const getSkillEffect = (id: string | null) => {
    ...
    if (id === 'sunBurst') {
      const dmg =
        uiState.plant.baseDamage
        * uiState.plant.damageMultiplier
        * 5
        * 1.15 ** (uiState.abilities.sunBurst.level - 1)
      ...
      const dmg =
        ...
        * uiState.plant.damageMultiplier
        * 2
        * 1.15 ** (uiState.upgrades.clickLevel - 1)
      return `${formatNumber(dmg)} Damage`
    } else if (id === 'energy') {
  ...
  const getSkillDescription = (id: string | null) => {
    switch (id) {
      case 'sunBurst':
        return 'Unleashes a burst of solar energy damaging all enemies on screen.'
      case 'rootEntangle':
        return 'Vines erupt from the ground slowing all enemies.'
      case 'poisonCloud':
        return 'Releases a toxic cloud dealing damage over time to all enemies.'
      case 'solGenerator':
        return 'Increases the amount of suns generated automatically.'
      case 'grass':
        return 'Sharp grass covers the ground dealing continuous damage to walking enemies.'
      case 'damage':
        return 'Increases the base damage of your plant.'
      case 'speed':
        return 'Increases the attack speed of your plant.'
      case 'click':
        return 'Increases the damage dealt when you click on enemies.'
      case 'energy':
        return 'Increases the amount of energy gained from all sources.'
      case 'evolutionSpeed':
        return 'Increases the speed at which your plant evolves.'
      default:
        return ''
    }
  }
  ...
  const evoPercent = Math.max(
    0,
    Math.min(100, (uiState.plant.evolutionProgress / requiredProgress) * 100),
  )
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-100 flex flex-col font-sans overflow-x-hidden select-none">
        ...
        <header className="bg-stone-900 border-b border-stone-800 p-4 flex justify-between items-center shadow-md z-10">
          <div className="flex items-center gap-2 md:gap-4">
            <div className="flex items-center gap-2 bg-stone-950 px-3 md:px-4 py-1.5 rounded-full border border-stone-800">
              ...
              <span className="font-mono text-lg md:text-xl font-bold text-yellow-400">
                {formatNumber(uiState.energy)}
              </span>
            </div>
          ...
          <div className="flex items-center gap-3 md:gap-6">
            <div className="text-center hidden xs:block">
              <div className="text-[9px] md:text-xs text-stone-500 font-bold uppercase tracking-wider">
                {t[lang].wave}
              </div>
            ...
            <div className="text-center">
              ...
              <div className="text-base md:text-xl font-black text-emerald-400">
          ...
          >
            ...
            >
              <div className="p-4 border-b border-stone-800 bg-stone-900/50 flex justify-between items-center">
                <h2 className="text-lg font-black tracking-tight text-stone-300">
                  {t[lang].upgrades}
                </h2>
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => setIsShopOpen(false)}
                    className="lg:hidden p-2 bg-stone-800 rounded-lg hover:bg-stone-700 text-stone-400 transition-colors"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                ...
                <div className="flex bg-stone-950 rounded-xl p-1 border border-stone-800">
                  {(['1', '10', 'MAX'] as const).map(mult => (
                    <button
                      key={mult}
                      onClick={() => setBuyMultiplier(mult === 'MAX' ? 'MAX' : parseInt(mult, 10))}
                      className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${
                        buyMultiplier === (mult === 'MAX' ? 'MAX' : parseInt(mult, 10))
      ...
      {/* Progress Panel */}
      {showProgressPanel && (
        <ProgressPanel lang={lang} uiState={uiState} onClose={() => setShowProgressPanel(false)} />
      )}

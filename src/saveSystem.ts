import { calculateMaxHp, createInitialState, type GameState } from './game'
...
function mapStateToSave(state: GameState): SaveData {
  return {
    ...
    skills: {
      sunBurst: {
        level: state.abilities.sunBurst.level,
        evolutions: state.abilities.sunBurst.evolutions,
      },
      rootEntangle: {
        level: state.abilities.rootEntangle.level,
        evolutions: state.abilities.rootEntangle.evolutions,
      },
      poisonCloud: {
        level: state.abilities.poisonCloud.level,
        evolutions: state.abilities.poisonCloud.evolutions,
      },
      solGenerator: {
        level: state.abilities.solGenerator.level,
        evolutions: state.abilities.solGenerator.evolutions,
      },
    },
    prestige: {
      resets: state.resets,
...
function mapSaveToState(save: SaveData): GameState {
  ...
  if (save.upgrades?.levels) {
    ...
    save.upgrades
    && typeof (save.upgrades as Record<string, unknown>).damageLevel === 'number'
  ) {
  ...
  if (hasPlantStats) {
    ...
    state.plant.attackSpeedMultiplier =
      save.plantStats!.attackSpeedMultiplier || state.plant.attackSpeedMultiplier
  } else {
    ...
    state.plant.baseDamage = 5 * 2 ** totalEvolutions
  }
...
function validateSaveData(data: unknown): boolean {
  ...
  if (progress) {
    if (
      ...
      || !Number.isFinite(progress.wave)
      || progress.wave < 0
      || progress.wave > 1e6
    )
      return false
    if (typeof progress.enemiesKilled !== 'number' && progress.enemiesKilled !== undefined)
      return false
  }
  ...
  if (plantStats) {
    if (
      typeof plantStats.baseDamage !== 'number'
      || !Number.isFinite(plantStats.baseDamage)
      || plantStats.baseDamage <= 0
    )
      return false
    if (
      typeof plantStats.damageMultiplier !== 'number'
      || !Number.isFinite(plantStats.damageMultiplier)
      || plantStats.damageMultiplier <= 0
    )
      return false
  }

import { CANVAS_H, CANVAS_W } from '../config/constants'
import type { GameState } from '../game'
import { drawCoins } from './draw-coins'
import { drawFloatingTexts, drawParticles, drawPortal, drawSunBursts } from './draw-fx'
...
export function drawGame(
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const y = (Math.cos(i * 678.9) * 0.5 + 0.5) * (internalH - 100)
  ...
  // Ground texture/grid
  const grassColor = state.upgrades.grassEvolutions.includes('poison_grass')
    ? '168, 85, 247'
    : '34, 197, 94'

...
export default function SkillEvolutionModal({ options, onSelect, lang }: SkillEvolutionModalProps) {
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-stone-900 border border-stone-700 rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl transform transition-all max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-stone-400 text-center text-sm mb-6">{t[lang].chooseUpgradePath}</p>
        <div className="flex flex-col gap-3">
          {options.map(opt => (
            ...
            >
              <div className="font-bold text-emerald-400 group-hover:text-emerald-300 mb-1">
                {opt.name}
              </div>
